...
const static int NUM_OF_TESTS_DYCK_MIRRORED = 1e5;
const static int TEST_MAX_EDGES_DYCK_MIRRORED = 1e4;
const static int TEST_MAX_BRANCHES_DYCK_MIRRORED = 1e2;

/**
 * Transform a dyck path to its mirrored pre-order tree representation.
 ...
 */
std::string get_random_dyck_path_mirrored(int deg, int len);

/**
 * Verify if a dyck path in mirrored pre-order is valid or not
 *
 * @param path:  A dyck path in mirrored pre-order
 *
...
/**
 * Find the `r` value for a given dyck path in mirrored pre-order. It assumes that
 * the given dyck path is valid.
 *
 * @param path:  A dyck path in mirrored pre-order
 *
 * @return r
